/*
This code is a variant of https://github.com/pbatard/rufus

It is a work in progress and currently not ready for distribution,
...
#define PF_INIT_OR_OUT(proc, name)      do {PF_INIT(proc, name);         \
  if (pf##proc == NULL) {printf("Unable to locate %s() in %s.dll: %s\n",  \
  #proc, #name, "kewlerror"); goto out;} } while(0)
...
static BOOL GetUSBProperties(char* parent_path, char* device_id, usb_device_props* props)
{
	...
	if (cr != CR_SUCCESS) {
		printf("Could not get device instance handle for '%s': CR error %d", device_id, cr);
		goto out;
	}
	...
	if (nWindowsVersion >= WINDOWS_8) {
		...
		if (!DeviceIoControl(handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX_V2, &conn_info_v2, size, &conn_info_v2, size, &size, NULL)) {
			printf("Could not get node connection information (V2) for device '%s':", device_id);
...
static BOOL GetDriveLabel(DWORD DriveIndex, char* letters, char** label)
{
	...
	*label = STR_NO_LABEL;
	...
	if (DeviceIoControl(hPhysical, IOCTL_STORAGE_CHECK_VERIFY, NULL, 0, NULL, 0, &size, NULL))
		...
		(HRESULT_CODE(GetLastError()) == ERROR_NOT_READY)?"No media":"Bad media");
	safe_closehandle(hPhysical);
...
//this is a big one.  in its modified form, it just returns to me the device number i want
/*
  ...
  for (i=0; SetupDiEnumDeviceInfo(dev_info, i, &dev_info_data); i++) {
    ...
    if (!SetupDiGetDeviceRegistryPropertyU(dev_info, &dev_info_data, SPDRP_FRIENDLYNAME,
      ...
      for (device_id = devid_list; *device_id != 0; device_id += strlen(device_id) + 1) {
        ...
        if ((uintptr_t)htab_devid.table[j].data > 0) {
          printf("  Matched with Hub[%d]: '%s'", (uintptr_t)htab_devid.table[j].data,
    ...
    if (props.is_VHD) {
      ...
      if (!list_non_usb_removable_drives) {
        ...
        printf("If you *REALLY* need, you can enable listing of this device with <Ctrl><Alt><F>");
        continue;
      }
      ...
      if ((props.vid == 0) && (props.pid == 0)) {
        if (!props.is_USB) {
          ...
          printf("Found non-USB non-removable device '%s' => Eliminated", buffer);
          continue;
        }
      ...
      if (props.is_LowerSpeed)
        printf("NOTE: This device is an USB 3.0 device operating at lower speed...");
    }
    devint_data.cbSize = sizeof(devint_data);
    ...
    for (j=0; ;j++) {
      ...
      if (!SetupDiEnumDeviceInterfaces(dev_info, &dev_info_data, &_GUID_DEVINTERFACE_DISK, j, &devint_data)) {
        if(GetLastError() != ERROR_NO_MORE_ITEMS) {
          ...
          printf("A device was eliminated because it didn't report itself as a disk\n");
        }
        break;
      }
      ...
      if (!SetupDiGetDeviceInterfaceDetailA(dev_info, &devint_data, NULL, 0, &size, NULL)) {
        if(GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
          ...
          if (devint_detail_data == NULL) {
            printf("Unable to allocate data for SP_DEVICE_INTERFACE_DETAIL_DATA\n");
            continue;
          }
          devint_detail_data->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA_A);
        } else {
      ...
      if (devint_detail_data == NULL) {
        printf("SetupDiGetDeviceInterfaceDetail (dummy) - no data was allocated\n");
        continue;
      }
      if(!SetupDiGetDeviceInterfaceDetailA(dev_info, &devint_data, devint_detail_data, size, &size, NULL)) {
      ...
      if (GetDriveLabel(drive_index, drive_letters, &label)) {
        if ((props.is_SCSI) && (!props.is_UASP) && (!props.is_VHD)) {
          ...
          if (!list_non_usb_removable_drives) {
            ...
            if (*p) {
              printf("Device eliminated because it contains a mounted partition that is set as non-removable");
              safe_closehandle(hDrive);
              safe_free(devint_detail_data);
              break;
            }
        ...
        if ((!enable_HDDs) && (!props.is_VHD) && (!props.is_CARD) &&
          ...
          printf("Device eliminated because it was detected as a Hard Drive (score %d > 0)", score);
          if (!list_non_usb_removable_drives)
            printf("If this device is not a Hard Drive, please e-mail the author of this application");
          printf("NOTE: You can enable the listing of Hard Drives in 'Advanced Options' (after clicking the white triangle)");
          safe_closehandle(hDrive);
          safe_free(devint_detail_data);
          break;
        }
 ...
 * Return INVALID_HANDLE_VALUE (/!\ which is DIFFERENT from NULL /!\) on failure.
 */
static HANDLE OldGetHandle(char* Path, BOOL bLockDrive, BOOL bWriteAccess, BOOL bWriteShare)
{
  int i;
  DWORD size;
  HANDLE hDrive = INVALID_HANDLE_VALUE;
...
// from format.c
static BOOL WriteDrive(HANDLE hPhysicalDrive, HANDLE hSourceImage, uint64_t sector_size, uint64_t drive_size)
{
	BOOL s, ret = FALSE;
	LARGE_INTEGER li;
  ...
  //return TRUE;
	if (!SetFilePointerEx(hPhysicalDrive, li, NULL, FILE_BEGIN))
		printf("Warning: Unable to rewind image position - wrong data might be copied!");
...
void Install(DeviceGuy * target_device, char * image_path) {
    ...
    HANDLE source_img = CreateFileU(image_path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    //TODO(kendall): make sure the handlers don't equal INVALID_HANDLE_VALUE
    safe_free(physical_path);
   if (phys_handle != INVALID_HANDLE_VALUE && source_img != INVALID_HANDLE_VALUE) {
