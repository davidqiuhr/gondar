...
// These calls take an int and char is signed so MS compilers will produce an
// assert error on anything that's > 0x80
...
// NB: other issomething() calls are not implemented as they may require
// multibyte UTF-8 sequences to be converted
...
static __inline DWORD FormatMessageU(DWORD dwFlags,
                                     LPCVOID lpSource,
                                     DWORD dwMessageId,
                                     DWORD dwLanguageId,
                                     char* lpBuffer,
  ...
  ret = FormatMessageW(dwFlags, lpSource, dwMessageId, dwLanguageId, wlpBuffer,
                       nSize, Arguments);
  err = GetLastError();
  if ((ret != 0) &&
      ((ret = wchar_to_utf8_no_alloc(wlpBuffer, lpBuffer, nSize)) == 0)) {
    err = GetLastError();
    ret = 0;
  }
  wfree(lpBuffer);
...
static __inline LRESULT SendMessageLU(HWND hWnd,
                                      UINT Msg,
                                      WPARAM wParam,
                                      const char* lParam) {
  LRESULT ret = FALSE;
...
static __inline int DrawTextExU(HDC hDC,
                                LPCSTR lpchText,
                                int nCount,
                                LPRECT lpRect,
                                UINT uFormat,
...
static __inline HWND CreateWindowU(char* lpClassName,
                                   char* lpWindowName,
                                   DWORD dwStyle,
                                   int x,
                                   int y,
  ...
  ret = CreateWindowW(wlpClassName, wlpWindowName, dwStyle, x, y, nWidth,
...
static __inline HWND CreateWindowExU(DWORD dwExStyle,
                                     char* lpClassName,
                                     char* lpWindowName,
                                     DWORD dwStyle,
                                     int x,
  ...
  wconvert(lpClassName);
  wconvert(lpWindowName);
  ret = CreateWindowExW(dwExStyle, wlpClassName, wlpWindowName, dwStyle, x, y,
...
static __inline int MessageBoxU(HWND hWnd,
                                LPCSTR lpText,
                                LPCSTR lpCaption,
                                UINT uType) {
  int ret;
...
static __inline int MessageBoxExU(HWND hWnd,
                                  LPCSTR lpText,
                                  LPCSTR lpCaption,
                                  UINT uType,
                                  WORD wLanguageId) {
...
static __inline int DrawTextU(HDC hDC,
                              LPCSTR lpText,
                              int nCount,
                              LPRECT lpRect,
                              UINT uFormat) {
...
static __inline UINT GetDlgItemTextU(HWND hDlg,
                                     int nIDDlgItem,
                                     char* lpString,
                                     int nMaxCount) {
  UINT ret = 0;
...
static __inline BOOL SetDlgItemTextU(HWND hDlg,
                                     int nIDDlgItem,
                                     const char* lpString) {
  BOOL ret = FALSE;
  DWORD err = ERROR_INVALID_DATA;
...
static __inline BOOL InsertMenuU(HMENU hMenu,
                                 UINT uPosition,
                                 UINT uFlags,
                                 UINT_PTR uIDNewItem,
...
static __inline int ComboBox_GetLBTextU(HWND hCtrl, int index, char* lpString) {
  ...
  size =
      (int)SendMessageW(hCtrl, CB_GETLBTEXT, (WPARAM)index, (LPARAM)wlpString);
  err = GetLastError();
  if (size > 0)
...
static __inline BOOL CopyFileU(const char* lpExistingFileName,
...
// This function differs from regular GetTextExtentPoint in that it uses a zero
// terminated string
static __inline BOOL GetTextExtentPointU(HDC hdc,
                                         const char* lpString,
                                         LPSIZE lpSize) {
  BOOL ret = FALSE;
  DWORD err = ERROR_INVALID_DATA;
...
static __inline DWORD GetCurrentDirectoryU(DWORD nBufferLength,
                                           char* lpBuffer) {
  DWORD ret = 0, err = ERROR_INVALID_DATA;
  walloc(lpBuffer, nBufferLength);
  ret = GetCurrentDirectoryW(nBufferLength, wlpBuffer);
  err = GetLastError();
  if ((ret != 0) && ((ret = wchar_to_utf8_no_alloc(wlpBuffer, lpBuffer,
                                                   nBufferLength)) == 0)) {
    err = GetLastError();
  }
  wfree(lpBuffer);
...
static __inline UINT GetSystemDirectoryU(char* lpBuffer, UINT uSize) {
  ...
  err = GetLastError();
  if ((ret != 0) &&
      ((ret = wchar_to_utf8_no_alloc(wlpBuffer, lpBuffer, uSize)) == 0)) {
    err = GetLastError();
  }
  wfree(lpBuffer);
  SetLastError(err);
...
static __inline DWORD GetTempFileNameU(char* lpPathName,
                                       char* lpPrefixString,
                                       UINT uUnique,
                                       char* lpTempFileName) {
  DWORD ret = 0, err = ERROR_INVALID_DATA;
  ...
  ret =
      GetTempFileNameW(wlpPathName, wlpPrefixString, uUnique, wlpTempFileName);
  err = GetLastError();
  if ((ret != 0) && ((ret = wchar_to_utf8_no_alloc(
                          wlpTempFileName, lpTempFileName, MAX_PATH)) == 0)) {
    err = GetLastError();
  }
...
static __inline DWORD GetModuleFileNameU(HMODULE hModule,
                                         char* lpFilename,
                                         DWORD nSize) {
  DWORD ret = 0, err = ERROR_INVALID_DATA;
...
static __inline DWORD GetFullPathNameU(const char* lpFileName,
                                       DWORD nBufferLength,
                                       char* lpBuffer,
                                       char** lpFilePart) {
  DWORD ret = 0, err = ERROR_INVALID_DATA;
...
static __inline int SHCreateDirectoryExU(HWND hwnd,
                                         const char* pszPath,
...
static __inline int SHDeleteDirectoryExU(HWND hwnd,
                                         const char* pszPath,
                                         FILEOP_FLAGS fFlags) {
  int ret;
  // String needs to be double NULL terminated, so we just use the length of the
  // UTF-8 string
  // which is always expected to be larger than our UTF-16 one, and add 2 chars
  // for good measure.
  size_t wpszPath_len = strlen(pszPath) + 2;
  walloc(pszPath, wpszPath_len);
  SHFILEOPSTRUCTW shfo = {hwnd,   FO_DELETE, wpszPath, NULL,
...
static __inline BOOL ShellExecuteExU(SHELLEXECUTEINFOA* lpExecInfo) {
  ...
  // Because we're lazy, we'll assume that the A and W structs inherently have
  // the same size
...
static __inline BOOL CreateProcessU(
  ...
  ret = CreateProcessW(wlpApplicationName, wlpCommandLine, lpProcessAttributes,
                       lpThreadAttributes, bInheritHandles, dwCreationFlags,
                       lpEnvironment, wlpCurrentDirectory, &wStartupInfo,
...
static __inline BOOL WINAPI GetOpenSaveFileNameU(LPOPENFILENAMEA lpofn,
                                                 BOOL save) {
  BOOL ret = FALSE;
  DWORD err = ERROR_INVALID_DATA;
  size_t i, len;
  ...
  if (lpofn->lpstrFilter != NULL) {
    ...
    // And revert
    len = wcslen(
        wofn.lpstrFilter);  // don't use in the loop as it would be reevaluated
    for (i = 0; i < len; i++) {
  ...
  utf8_to_wchar_no_alloc(lpofn->lpstrFileTitle, wofn.lpstrFileTitle,
                         lpofn->nMaxFileTitle);
  ...
  err = GetLastError();
  if ((ret) &&
      ((wchar_to_utf8_no_alloc(wofn.lpstrFile, lpofn->lpstrFile,
                               lpofn->nMaxFile) == 0) ||
       (wchar_to_utf8_no_alloc(wofn.lpstrFileTitle, lpofn->lpstrFileTitle,
...
extern BOOL WINAPI UpdateDriverForPlugAndPlayDevicesW(HWND hwndParent,
                                                      LPCWSTR HardwareId,
                                                      LPCWSTR FullInfPath,
                                                      DWORD InstallFlags,
                                                      PBOOL bRebootRequired);

static __inline BOOL UpdateDriverForPlugAndPlayDevicesU(HWND hwndParent,
                                                        const char* HardwareId,
                                                        const char* FullInfPath,
                                                        DWORD InstallFlags,
                                                        PBOOL bRebootRequired) {
  BOOL ret = FALSE;
  DWORD err = ERROR_INVALID_DATA;
  ...
  ret = UpdateDriverForPlugAndPlayDevicesW(
      hwndParent, wHardwareId, wFullInfPath, InstallFlags, bRebootRequired);
  err = GetLastError();
  wfree(HardwareId);
  wfree(FullInfPath);
  SetLastError(err);
...
static __inline BOOL SetupCopyOEMInfU(const char* SourceInfFileName,
                                      const char* OEMSourceMediaLocation,
  ...
  if (DestinationInfFileNameComponent != NULL)
    goto out;

  ret = SetupCopyOEMInfW(wSourceInfFileName, wOEMSourceMediaLocation,
                         OEMSourceMediaType, CopyStyle, wDestinationInfFileName,
                         DestinationInfFileNameSize, RequiredSize, NULL);
  err = GetLastError();
  if ((ret != FALSE) && ((ret = wchar_to_utf8_no_alloc(
                              wDestinationInfFileName, DestinationInfFileName,
                              DestinationInfFileNameSize)) == 0)) {
    err = GetLastError();
  }
out:
