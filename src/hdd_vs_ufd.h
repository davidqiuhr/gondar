 ...
 * Copyright © 2008-13 Christian Franke
...
/* String identifiers:
 * Some info comes from
 * http://knowledge.seagate.com/articles/en_US/FAQ/204763en,
 * other
 * http://svn.code.sf.net/p/smartmontools/code/trunk/smartmontools/drivedb.h
...
static str_score_t str_score[] =
    {
        {"IC#", 10},      {"ST#", 10},        {"MX#", 10},
        {"WDC", 10},      {"IBM", 10},        {"STM#", 10},
        {"HDS#", 10},  // These Hitachi drives are a PITA
        {"HDP#", 10},     {"HDT#", 10},       {"HTE#", 10},
        {"HTS#", 10},     {"HUA#", 10},       {"APPLE", 10},
        {"INTEL", 10},    {"MAXTOR", 10},     {"HITACHI", 10},
        {"SEAGATE", 10},  {"SAMSUNG", 5},     {"FUJITSU", 10},
        {"TOSHIBA", 5},   {"QUANTUM", 10},    {"EXCELSTOR", 10},
        {"CORSAIR", -15}, {"KINGMAX", -15},   {"KINGSTON", -15},
        {"LEXAR", -15},   {"MUSHKIN", -15},   {"PNY", -15},
        {"SANDISK", -15}, {"TRANSCEND", -15},
};
 ...
 * 1. Pick all the VID:PIDs from
 * http://svn.code.sf.net/p/smartmontools/code/trunk/smartmontools/drivedb.h
 * 2. Check that VID against http://flashboot.ru/iflash/saved/ as well as
 * http://www.linux-usb.org/usb.ids
 * 3. If a lot of flash or card reader devices are returned, add the VID:PID,
 * with a positive score,
 *    in the vidpid table (so that the default will be UFD, and HDD the
 * exception)
 * 4. If only a few flash devices are returned, add the VID to our list with a
 * positive score and
 *    add the flash entries in the VID:PID list with a negative score
 * 5. Add common UFD providers from http://flashboot.ru/iflash/saved/ with a
 * negative score
...
static vidpid_score_t vidpid_score[] = {
    ...
    {0x11b0, 0x6298,
     10},  // Enclosure from Kingston SSDNow notebook upgrade kit
    // NOT in VID list as plenty of UFDs
    ...
    {0x0325, 0xac02,
     -20},  // OCZ ATV Turbo / Rally2 Dual Channel USB 2.0 Flash Drive
    // Buffalo exceptions
